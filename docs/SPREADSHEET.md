...
- **Edit mode** swaps the active cell to its `<input>` and gives it focus. Enter it with **F2**, **double-click**, or by typing a printable character (the character replaces the current value as the seed). **Backspace** with no range, on a populated cell, clears the cell and stays in navigation. **Escape** reverts a pending edit and returns to navigation; **Tab** / **Enter** commit the edit and move (Enter returns to the Tab-anchor column on the next row). Plain **Arrow** keys (no modifiers) also commit and move to the adjacent cell — they do not move the text caret within the input.
